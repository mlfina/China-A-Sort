...
for char in tqdm(char_list):
    ...
    # da['Trdmnt'] = da['Trdmnt'] + MonthEnd(1) # add this line, then get predictive table; comment this line, then get t2t table.
    da = da[ (da['Trdmnt']>='2000') & (da['Trdmnt']<='2023')]
    df_melt = da.melt(id_vars=['Trdmnt'],value_name =char)
    df_melt.columns = ['date','asset',char]
    outputpath="../data/tmp_t2t/"+char+".csv"
    df_melt.to_csv(outputpath,sep=',',index=False,header=True)


da = df.copy()
for char in tqdm(char_list):
    s1 = pd.read_csv("../data/tmp_t2t/"+char+".csv")
    #,encoding='utf8',error_bad_lines=False, engine ='python')
...
da.to_csv("../data/output/panel_raw_t2t.csv")
...
da_rank.to_csv("../data/output/panel_rank_t2t.csv")
